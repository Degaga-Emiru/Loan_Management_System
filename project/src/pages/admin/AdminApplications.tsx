...
// -------- OPTIONAL: APPLIED DATE FORMATTER --------
// UNCOMMENT TO ENABLE when you return an "appliedDate" (or similar) from the API.
...
//     const ms = asNumber < 1e12 ? asNumber * 1000 : asNumber; // seconds -> ms
//     const d = new Date(ms);
//     return isNaN(d.getTime()) ? "—" : d.toLocaleDateString();
//   }
//   const d = new Date((input as string).replace(" ", "T"));
//   return isNaN(d.getTime()) ? "—" : d.toLocaleDateString();
// };
              ...
              <table className='min-w-full divide-y divide-gray-200'>
                <thead className='bg-gray-50'>
                  <tr>
                    ...
                    {/* -------- OPTIONAL: APPLIED COLUMN (HEADER) --------
                    UNCOMMENT TO ENABLE when your API returns r.appliedDate (string|number)
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      Applied
                    </th>
                    */}
                ...
                <tbody className='bg-white divide-y divide-gray-200'>
                  {rows.map((r) => (
                    <tr key={r.id} className='hover:bg-gray-50'>
                      ...
                      {/* -------- OPTIONAL: APPLIED COLUMN (CELL) --------
                      UNCOMMENT BOTH THIS <td> AND THE HEADER ABOVE
                      AND make sure fetchPendingForAdmin() returns { appliedDate }
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {formatDate(r.appliedDate as any)}
                      </td>
                      */}
